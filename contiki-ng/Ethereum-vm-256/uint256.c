...
void not128(uint128_t *number) {
    UPPER_P(number) = ~UPPER_P(number) ;
    LOWER_P(number) = ~LOWER_P(number) ;
}

void not256(uint256_t *number) {
    not128(&UPPER_P(number));
    not128(&LOWER_P(number));
}
